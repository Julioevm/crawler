...
class PlayingState(BaseState):
    def __init__(self, game):
        ...
        self.raycaster.set_player_position(self.player.x, self.player.y)
        self.raycaster.set_player_angle(self.player.angle)

        self.turn_manager = TurnManager(self.game_map)
        self.combat_manager = CombatManager()

        self.ui = UI(SCREEN_WIDTH, SCREEN_HEIGHT, show_fps=True)
    ...
    def load_level(self, file_path):
        ...
        for item_data in level_data.get("items", []):
            ...
            if isinstance(item, Weapon) and not self.player.equipped_weapon:
    ...
    def get_event(self, event):
        if event.type == KEYDOWN:
            ...
            if event.key == K_i and not self.combat_manager.in_combat:
                new_state = InventoryState(self.player)
    ...
    def handle_player_input(self, event):
        ...
        if event.key == K_w or event.key == K_UP:
            ...
            dx = math.cos(self.player.angle - math.pi/2)
            dy = math.sin(self.player.angle - math.pi/2)
        ...
        if (dx != 0 or dy != 0):
            target_x = int(self.player.x + round(dx))
            target_y = int(self.player.y + round(dy))

            if self.game_map.is_walkable(target_x, target_y):
                if int(self.player.x) != target_x or int(self.player.y) != target_y:
            ...
            entities_at_position = self.game_map.get_entities_at(self.player.x, self.player.y)
            for entity in entities_at_position:
                if isinstance(entity, Enemy) and entity.is_alive():
                    new_state = CombatState(self.game, self.player, entity, self.combat_manager)
                    self.game.push_state(new_state)
                    break
    ...
    def draw(self, screen, clock):
        ...
        self.ui.draw_player_stats(screen, self.player)
        self.ui.draw_messages(screen, self.messages)
        self.ui.draw_fps(screen, clock.get_fps())
        
        self.minimap_ui.draw(screen, self.game_map, self.player)